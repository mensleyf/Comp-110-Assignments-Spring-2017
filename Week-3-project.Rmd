...
```{r}
ggplot(data = NCHS, aes(x = bmi)) + geom_density(aes(color="red",fill=TRUE))+ facet_wrap(sex~pregnant)
```


I haven't figured out how to make the graphic that appears in the in class example yet..it has colored density according to gender, and overlays non-pregnnt females and makes.
...
PLaying with graphs 
```{r}
ggplot(data = NCHS, aes(x = income)) + geom_density(aes(color="blue",fill=TRUE))+ facet_wrap(sex~pregnant)
```
...
```
Second choice selections? 10,598
